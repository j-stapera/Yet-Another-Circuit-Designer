...
func _on_port_clicked(component_id: int, terminal_name: String, position: Vector2):
	if not placing_wire and wire_mode:
		start = { "id": component_id, "terminal": terminal_name }
		...
		end = { "id": component_id, "terminal": terminal_name }
